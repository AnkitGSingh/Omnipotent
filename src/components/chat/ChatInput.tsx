...
export function ChatInput({
  ...
  return (
    <div
      className="absolute bottom-0 left-0 right-0 w-full max-w-3xl mx-auto px-6 pb-8 z-20 pt-4"
      style={{
        background:
        ...
        <div className="px-4 pb-4 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <motion.button
              ...
              className="p-1.5 text-foreground/40 hover:text-primary transition-colors rounded-lg hover:bg-primary/10"
          ...
          <div className="flex items-center gap-3">
            {isLoading && (
              <span className="text-[10px] font-mono text-primary/70 animate-pulse">
                generating...
              </span>
            )}
            <motion.button
              ...
              onClick={() => onSubmit()}
              disabled={!canSend}
              whileHover={canSend ? { y: -2, scale: 1.06 } : {}}
              whileTap={canSend ? { scale: 0.9, y: 0 } : {}}
              transition={{ type: "spring", stiffness: 420, damping: 18 }}
              className="flex items-center justify-center w-8 h-8 rounded-xl disabled:opacity-30 disabled:cursor-not-allowed"
              style={{
                backgroundColor: canSend
                  ? "hsl(var(--primary))"
                  : "hsl(var(--primary)/0.15)",
                boxShadow: canSend ? "0 0 16px hsl(var(--primary)/0.35)" : "",
              }}
            >
              <ArrowUp
                size={14}
